...
#define ALMOST_WIN	 0 //INT_MAX - 100000
#define ALMOST_LOSE  0 //INT_MIN + 100000
...
#define VAL_FUNCTION 												  \
		...
		{\
			if(marks[my_pawn]>marks[opponent_pawn])return marks[my_pawn];\
				return 9;				\
		}\
		\
...
{
	//std::cout << "E:"<<dummy_evaluation(, get_game_info(),0) << std::endl;
	int move_tmp = findBestMove(get_board(), get_mark());
	std::cout << "Bot's move:" << move_tmp << std::endl;
	set_move(move_tmp);
}
...
int Bot::evaluation(Board *Board, uint8_t win_mark_amount, uint8_t turn)
{
	...
	uint8_t n = Board->get_size();				//size of battle_field
	uint8_t my_pawn = get_mark();				//bot's pawn
	uint8_t opponent_pawn = (my_pawn==1)? 2:1;	//opponent's pawn
	float result;
    ...
    for(int y = 0; y<Board->get_size(); y++)
    {
        for (int x = 0; x<Board->get_size(); x++)
        {
        	...
        	if (Board->get_field(x,y)==0)
            {
                ...
                // move.
				int dp;
				if(Board->get_size()>6)dp=5;
				else
				{
					dp=5;
				}
				
                int val_tmp = minmax(Board, dp, false,-100000,100000);
