  ...
  public:
	...
	Vertex* insertVertex(W O)
	{
		...
		if(vertices_amount > adjency_matrix.size())
			adjency_matrix.resize(adjency_matrix.size()+1);
	...
	void displayIncidentEdges(Vertex* V)
	{
		...
		{
			std::cout << "Address: " << edges_tmp[i];
			if(edges_tmp[i] != 0)
				std::cout << " Data: " << edges_tmp[i]->weight << std::endl;
			else
	...
	void displayVertices(void)
	{
		...
		{
			vertex_tmp = *i;
			std::cout << "Index: " << vertex_tmp->idx << " Address: " << vertex_tmp << " Data: " << vertex_tmp->data << std::endl;
		}
	}
	...
	void dijkstra(void)
	{
		...
		while(!Q.empty())
		{
			auto u = Q.removeMin(); // u to index vertexa 
				...
				Edge** e_val = incidentEdges(map[u]);// ? ale bardziej ok 
			for(int e = 0; e < vertices_amount; e++)	//for all e e G.indicentEdges(u)
			{
				...
				if(e_val[e] != 0)
				{
					...
					if(r < distance[z->idx])
					{
						...
						Q.replaceKey(locator[z->idx], z->idx /*, locator*/);
					}
				}
		...
		std::cout << "source: " << v->idx << std::endl;
		for(int i = 0; i < vertices_amount; i++)
